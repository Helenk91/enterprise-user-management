/**
 * 中国行政区划数据（省/自治区/直辖市/特别行政区 → 地级市/自治州/地区/区县）
 * 用于「地区」二级联动选择（省份 → 城市），覆盖全部 34 个省级行政区
 */
...
export const REGIONS = [
  ...
  { name: '重庆市', cities: ['渝中区', '大渡口区', '江北区', '沙坪坝区', '九龙坡区', '南岸区', '北碚区', '渝北区', '巴南区', '涪陵区', '长寿区', '江津区', '合川区', '永川区', '南川区', '綦江区', '大足区', '璧山区', '铜梁区', '潼南区', '荣昌区', '开州区', '梁平区', '武隆区', '万州区', '黔江区', '城口县', '丰都县', '垫江县', '忠县', '云阳县', '奉节县', '巫山县', '巫溪县', '石柱土家族自治县', '秀山土家族苗族自治县', '酉阳土家族苗族自治县', '彭水苗族土家族自治县'] },
  ...
  { name: '澳门特别行政区', cities: ['澳门半岛', '氹仔岛', '路环岛'] },
]

/** 获取某省份下的城市列表 */
export function getProvinceCities(provinceName) {
  const p = REGIONS.find((x) => x.name === provinceName)
  return p ? p.cities : []
}

/**
 * 解析地区存储值 → { province, city }
 * 兼容新格式（"广东省 广州市"）与历史数据（"上海"、"广州" 等旧值）
 */
export function parseRegion(value) {
  if (!value) return { province: '', city: '' }
  const v = String(value).trim()
  if (!v) return { province: '', city: '' }

  // 1) 新格式："省份 城市"
  const idx = v.indexOf(' ')
  if (idx > -1) {
    const prov = v.slice(0, idx).trim()
    const city = v.slice(idx + 1).trim()
    const p = REGIONS.find((x) => x.name === prov)
    if (p && p.cities.includes(city)) return { province: prov, city }
  }

  // 2) 城市精确匹配："广州市" / "海淀区" / "大理白族自治州"
  for (const p of REGIONS) {
    if (p.cities.includes(v)) return { province: p.name, city: v }
  }

  // 3) 省级精确匹配："广东省" / "北京市"
  for (const p of REGIONS) {
    if (v === p.name) return { province: p.name, city: '' }
  }

  // 4) 历史旧值：城市不带"市"后缀，如 "广州"→"广州市"、"上海"→"上海市"
  for (const p of REGIONS) {
    if (p.cities.includes(v + '市')) return { province: p.name, city: v + '市' }
    if (v + '市' === p.name) return { province: p.name, city: '' }
  }

  // 5) 历史旧值：省级简称，如 "广东"→"广东省"、"内蒙古"→"内蒙古自治区"
  for (const p of REGIONS) {
    const short = p.name.replace(/维吾尔自治区|壮族自治区|回族自治区|特别行政区|自治区|省|市/g, '')
    if (v === short) return { province: p.name, city: '' }
  }
